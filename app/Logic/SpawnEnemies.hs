...
makeRandomAsteroid g = 
    ...
    in
        (MkAsteroid{
        asteroidPosition    = wrapPoint randomPosition,
...
getRandomSize :: StdGen -> (Size,StdGen)
getRandomSize g = let (r, g') = randomR (0,100) g  :: (Int, StdGen) in           
                  let size    =  if r < 5  then ExtraLarge --5%  chance
                            else if r < 20 then Large      --15% chance
                            else if r < 40 then Small      --20% chance
                            else                Medium     --60% chance
                in (size,g')
